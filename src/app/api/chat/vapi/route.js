  ...
  try {
    ...
    if (!process.env.NEXT_PUBLIC_VAPI_API_KEY) {
      return NextResponse.json({ 
        message: 'VAPI not configured. Please set NEXT_PUBLIC_VAPI_API_KEY in environment variables.' 
      }, { status: 500 });
    }

    // Use VAPI for text-based conversation
    const response = await fetch('https://api.vapi.ai/assistant/chat', {
      ...
      body: JSON.stringify({
        ...
        message: message,
        userId: userId || requestUserId || 'guest',
        stream: false
      })
    });
    ...
    return NextResponse.json({
      response: data.message || data.response || "I'm sorry, I couldn't process your request at the moment.",
    ...
    // Fallback to simple responses if VAPI fails
